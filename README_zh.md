...
4. 点击 **Daily AI Trending Report → Run workflow** 手动触发一次，验证是否正常

之后每天 UTC 08:00（北京时间 16:00）会自动运行：
- 报告出现在 `reports/` 目录下
- 看板自动更新到 `https://<your-username>.github.io/github-ai-radar/`

> GitHub Actions 无需配置个人访问令牌或 LLM API Key。工作流会使用任务级 `GITHUB_TOKEN` 访问 GitHub API 和 GitHub Models；`daily.yml` 已声明所需的 `contents`、`pages`、`id-token` 和 `models` 权限。

如需改用其他 OpenAI 兼容服务，可在 **Settings → Secrets and variables → Actions** 中添加可选的 `LLM_API_KEY`、`LLM_API_BASE` 和 `LLM_MODEL`。
...
英文简介来自仓库官方 GitHub About 与经过过滤、评分的 README 核心内容；中文简介是对这些资料的忠实翻译，不会额外编造营销式描述。

GitHub Actions 默认通过任务级 `GITHUB_TOKEN` 使用 GitHub Models，因此 Fork 后无需添加额外 Secret。本地运行或使用自定义 OpenAI 兼容服务时，可设置：
...
翻译服务不可用时，看板仍会展示基于官方 About/README 的准确英文简介，不会凭空生成内容。
...
- 简介来源标签：`About`、`README` 或 `About + README`
- `中文` / `EN` 标签会切换真实的 `intro_zh` 与 `intro_en` 字段
- NEW 标记（7 天内创建的新项目）
- 排名变化指示器（↑ 上升 / ↓ 下降 / ★ 新上榜）
...
└── .github/workflows/
    ├── daily.yml                      ← 每日生成、提交并部署 Pages
    └── pages.yml                      ← 手动/docs push 部署的备用流程
```
